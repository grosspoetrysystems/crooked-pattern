...
export function markdownReport(artifact: ArsArtifact) {
  ...
  const findings = artifact.checks
    ...
    .slice(0, 10);
  const disagreements = artifact.checks.filter((check) => check.agreement_state === "disagree");
  ...
  return [
    ...
    "## Category Scores",
    "",
    ...Object.entries(s.categories).map(([name, value]) => `- ${label(name)}: ${value.result === "assessed" ? `${value.score}/100` : "unassessed"}`),
    "",
    "## Disagreements",
    "",
    disagreements.length ? table(disagreements) : "No source/wire disagreements were recorded in this run.",
    "",
    "## Prioritized Recommendations",
    "",
    findings.length ? findings.map((check) => `- **${check.title}** (${check.id}): ${check.notes.join(" ")}`).join("\n") : "No failing or partial deterministic checks.",
    "",
    "## Check Results",
    "",
    ...
    ...heuristicChecks(artifact).map((check) => `- ${check.title} is currently marked ${check.metadata?.confidence}/${check.metadata?.status}; treat it as a deterministic heuristic, not definitive proof.`),
    ""
  ].join("\n");
}
...
function table(checks: CheckResult[]) {
  return [
    ...
    "|---|---:|---:|---|---|---|---|",
    ...checks.map((check) => `| ${escapeCell(check.title)} | ${check.result} | ${check.score} | ${check.mode} | ${check.deterministic ? "yes" : "no"} | ${escapeCell(formatMetadata(check))} | ${escapeCell(check.notes.join(" "))} |`)
  ].join("\n");
}

function heuristicChecks(artifact: ArsArtifact) {
  return artifact.checks.filter((check) => check.metadata?.confidence === "heuristic");
}

function formatMetadata(check: CheckResult) {
  const parts = [];
  if (check.metadata?.confidence) parts.push(`confidence=${check.metadata.confidence}`);
  if (check.metadata?.status) parts.push(`status=${check.metadata.status}`);
  if (check.metadata?.labels?.length) parts.push(`labels=${check.metadata.labels.join(",")}`);
  return parts.join("; ");
}
