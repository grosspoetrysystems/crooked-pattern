...
const cooldownFiles = [".npmrc", ".yarnrc.yml", "pnpm-workspace.yaml", "bunfig.toml", "renovate.json", ".github/dependabot.yml"];

export async function runSourcePass(root: string): Promise<CheckResult[]> {
  const results: CheckResult[] = [];
  const packageJson = await readJson(path.join(root, "package.json"));
  const files = await listFiles(root);
  const directDeps = packageJson ? Object.keys({ ...(packageJson.dependencies ?? {}), ...(packageJson.devDependencies ?? {}) }) : [];
  ...
  results.push(check(
    ...
    packageJson ? 100 : 0,
    [packageJson ? `Found package.json with ${directDeps.length} direct dependencies.` : "No package.json found; non-Node projects are not fully implemented yet."],
    { source_value: packageJson ? { direct_dependencies: directDeps.length } : undefined }
  ));
  ...
  results.push(check(
    ...
    [foundLockfiles.length > 0 ? `Found ${foundLockfiles.join(", ")}.` : "No supported lockfile found; deterministic installs are not evidenced."],
  ...
  const deterministicInstall = /\bnpm ci\b|\bpnpm install --frozen-lockfile\b|\byarn install --immutable\b|\bbun install --frozen-lockfile\b/.test(ciText);
  results.push(check(
    ...
    [deterministicInstall ? "CI uses a deterministic install command." : ciFiles.length ? "CI workflows found, but no deterministic install command was detected." : "No GitHub Actions workflows found."],
    { source_value: { workflows: ciFiles.length } }
  ));

  const sca = /\bosv-scanner\b|\bsocket\b|\bsemgrep\b|dependency-review-action/.test(ciText);
  results.push(check(
    "source.sca_gate",
    "SCA gate in CI",
    "supply_chain_safety",
    ...
    sca ? 100 : 0,
    [sca ? "Detected OSV, Socket, Semgrep, or dependency review usage." : ciFiles.length ? "No SCA gate detected in CI workflows." : "No CI workflows available to inspect."],
  ));

  const provenance = /npm publish --provenance|slsa|sigstore|cosign|npm provenance/i.test(ciText);
  results.push(check(
    ...
    provenance ? 100 : 0,
    [provenance ? "Detected provenance/signing terms in CI." : "No SLSA, Sigstore, cosign, or npm provenance evidence found."]
  ));

  const sbom = [...files].some((file) => /(^|\/)(sbom|bom)\.(json|xml|spdx|cdx)|\.spdx\.json$|\.cdx\.json$/i.test(file));
  results.push(check(
    "source.sbom",
    "SBOM present",
    "supply_chain_safety",
  ...
  results.push(check(
    ...
    [slop.length ? `Suspicious dependency names: ${slop.join(", ")}.` : directDeps.length ? "No static dependency-name collision flags found." : "No dependency inventory available."],
  ...
  results.push(check(
    ...
    [authoredToolEvidence.tools.length ? `Parsed authored tools: ${authoredToolEvidence.tools.join(", ")}.` : "No parseable authored MCP/WebMCP/OpenAPI tool definitions found."],
...
async function listFiles(root: string, dir = ""): Promise<Set<string>> {
  ...
  for (const entry of await readdir(abs)) {
    if (["node_modules", ".git", "dist", "coverage", ".tempor", ".pnpm-store", "fixtures"].includes(entry)) continue;
...
async function detectAuthoredTools(root: string, files: Set<string>) {
  const evidence: { tools: string[]; sources: string[] } = { tools: [], sources: [] };
  for (const file of files) {
    if (!isToolDefinitionCandidate(file)) continue;
    const text = await readFileSafe(path.join(root, file));
    if (!text) continue;
    const tools = file.endsWith(".json") ? toolsFromJson(text) : toolsFromCode(text);
    if (tools.length) {
...
function isToolDefinitionCandidate(file: string) {
  return /(^|\/)(mcp|webmcp|server-card|openapi|api-catalog|tools?)[^/]*\.(json|ts|tsx|js|mjs|cjs)$/i.test(file);
}
...
function toolsFromJson(text: string) {
  ...
  if (parsed?.paths && typeof parsed.paths === "object") {
    for (const [route, methods] of Object.entries(parsed.paths)) {
      ...
      for (const method of Object.keys(methods)) tools.add(`${method.toUpperCase()} ${route}`);
    }
  }
  return [...tools];
}
...
async function detectCooldown(root: string, files: Set<string>) {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      const days = match[1] === "minimumReleaseAge" ? rawValue / 1440 : rawValue;
      return { days, files: relevant, note: `Detected dependency cooldown/minimum-release-age of ${formatDays(days)} day(s).` };
    }
  }
  return { days: 0, files: relevant, note: relevant.length ? "Cooldown config files inspected, but no supported minimum-age value was detected." : "No cooldown/minimum-release-age config found." };
}
...
function isSuspiciousName(dep: string) {
  const popular = ["react", "express", "lodash", "axios", "commander", "typescript", "next", "vite"];
  const normalized = dep.replace(/^@[^/]+\//, "").toLowerCase();
  return popular.some((name) => normalized !== name && distance(normalized, name) === 1);
}
