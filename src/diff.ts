...
export async function diffArtifacts(beforePath: string, afterPath: string) {
  ...
  for (const [category, afterScore] of Object.entries(after.summary.categories)) {
    ...
    lines.push(`- ${category}: ${formatCategory(beforeScore)} -> ${formatCategory(afterScore)} (${delta(afterValue - beforeValue)})`);
  }
  lines.push("", "## Checks", "");
  ...
  for (const check of after.checks) {
    ...
    if (!old || old.score !== check.score || old.result !== check.result) {
      lines.push(`- ${check.id}: ${old?.result ?? "missing"} ${old?.score ?? 0} -> ${check.result} ${check.score} (${delta(check.score - (old?.score ?? 0))})`);
    }
  }
  return lines.join("\n");
}

function categoryScore(value: { result: "assessed" | "unassessed"; score?: number } | undefined) {
  return value?.result === "assessed" ? value.score ?? 0 : 0;
}

function formatCategory(value: { result: "assessed" | "unassessed"; score?: number } | undefined) {
