...
describe("CLI integration pipeline", () => {
  ...
  afterAll(async () => {
    await Promise.all([closeServer(secure?.server), closeServer(insecure?.server)]);
  });
  ...
  it("writes source-only JSON and markdown artifacts with unassessed wire categories", async () => {
    ...
    expect(artifact.summary.measured_categories).toBeLessThan(artifact.summary.total_categories);
    expect(artifact.summary.categories.agent_operability?.result).toBe("assessed");
  ...
  it("scores secure and insecure wire fixtures through the built CLI", async () => {
    ...
    expect(findCheck(secureArtifact, "wire.llms_txt_present")?.result).toBe("pass");
    expect(findCheck(secureArtifact, "wire.initial_html_content")?.metadata?.confidence).toBe(
      "heuristic"
    );
    expect(secureArtifact.summary.exposure_multiplier).toBe(1);

    const liveTools = findCheck(insecureArtifact, "wire.mcp_server_card")?.wire_value as
    ...
    expect(findCheck(insecureArtifact, "wire.rule_of_two")?.result).toBe("fail");
...
async function serveFixture(directory: string): Promise<FixtureServer> {
  const server = createServer(async (request, response) => {
    ...
    const safePath = path
      ...
      .replace(/^(\.\.[/\\])+/, "");
    const filePath = path.join(directory, safePath === "/" ? "index.html" : safePath);
